dnl
dnl This file was generated by GATBPS 0.1.0-5995+g317175b, which was
dnl released on 2023-12-03. Before changing it, make sure
dnl you're doing the right thing. Depending on how GATBPS
dnl is being used, your changes may be automatically lost.
dnl A short description of this file follows.
dnl
dnl Special file: GATBPS_DEFINE_CLASSPATH.m4
dnl
dnl For more information, see the GATBPS manual.
dnl
#serial 20231203
AC_DEFUN([GATBPS_DEFINE_CLASSPATH], [[{

#
# The block that contains this comment is the expansion of the
# GATBPS_DEFINE_CLASSPATH macro.
#]dnl
m4_ifdef(
  [GATBPS_DEFINE_CLASSPATH_HAS_BEEN_CALLED],
  [gatbps_fatal([
    GATBPS_DEFINE_CLASSPATH has already been called
  ])],
  [m4_define([GATBPS_DEFINE_CLASSPATH_HAS_BEEN_CALLED])])[]dnl
m4_if(
  m4_eval([$# != 0]),
  [1],
  [gatbps_fatal([
    GATBPS_DEFINE_CLASSPATH requires exactly 0 arguments
    ($# ]m4_if([$#], [1], [[was]], [[were]])[ given)
  ])])[]dnl
[

case "$][{CLASSPATH+is_set}" in
  ?*)
  ;;
  *)
    CLASSPATH=''\''.'\'''
  ;;
esac
'readonly' 'CLASSPATH'

]AC_ARG_VAR(
  [CLASSPATH],
  [
    the class path to use with all Java tools that use a class path
    (default: CLASSPATH=''\''.'\''').
    This must be either empty or a shell word, and it is best to
    maximally quote the shell word to prevent surprising behavior.
    For example, you might use CLASSPATH=''\''/usr/share/java/*'\''' to
    search for classes and resources in all JAR files one level below
    the /usr/share/java directory.
    If you were to use CLASSPATH='/usr/share/java/*', then the shell
    would perform pattern matching in certain situations, which is
    probably not what you want.
  ])[

:;}]])[]dnl
dnl
dnl The authors of this file have waived all copyright and
dnl related or neighboring rights to the extent permitted by
dnl law as described by the CC0 1.0 Universal Public Domain
dnl Dedication. You should have received a copy of the full
dnl dedication along with this file, typically as a file
dnl named <CC0-1.0.txt>. If not, it may be available at
dnl <https://creativecommons.org/publicdomain/zero/1.0/>.
dnl
