...
#]dnl
m4_ifdef(
  [DEFINE_JDEPS_HAS_BEEN_CALLED],
  [gatbps_fatal([
    DEFINE_JDEPS has already been called
  ])],
  [m4_define([DEFINE_JDEPS_HAS_BEEN_CALLED])])[]dnl
m4_if(
  m4_eval([$# != 0]),
  [1],
  [gatbps_fatal([
    DEFINE_JDEPS requires exactly 0 arguments
    ($# ]m4_if([$#], [1], [[was]], [[were]])[ given)
  ])])[]dnl
[

]AC_REQUIRE([GATBPS_PROG_JDEPS])[
